...
export function GradesLegend({ labels }: GradesLegendProps) {
  const statusItems = [
    ...
    { c: "bg-iu-blue", label: labels.statusRegistered, icon: Clock },
    { c: "bg-iu-purple", label: labels.statusExamRegistered, icon: Calendar },
    { c: "bg-iu-orange", label: labels.statusOpen, icon: Clock },
    { c: "bg-iu-red", label: labels.statusFailed, icon: AlertCircle },
  ];

  const gradeItems = [
    { c: "bg-iu-blue", label: labels.gradeExcellent },
    { c: "bg-iu-purple", label: labels.gradeGood },
    { c: "bg-iu-orange", label: labels.gradeSatisfactory },
    { c: "bg-muted-foreground", label: labels.gradeSufficient },
  ];
    ...
    <div className="bg-card/60 backdrop-blur-xl rounded-xl sm:rounded-2xl border border-border p-4 sm:p-6 shadow-lg">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-2 sm:gap-3">
        {statusItems.map((it, idx) => (
          ...
          >
            ...
            <div className="text-[8px] sm:text-[9px] font-bold uppercase tracking-wide text-muted-foreground leading-tight">
              {it.label}
            </div>
          </div>
        ))}
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-3 mt-4 sm:mt-6">
        {gradeItems.map((it, idx) => (
          ...
          >
            <div
              className={`w-3 h-3 sm:w-4 sm:h-4 rounded ${it.c} shadow group-hover:scale-125 transition-transform`}
            />
            <div className="text-[8px] sm:text-[9px] font-bold uppercase tracking-wide text-muted-foreground">
              {it.label}
            </div>
          </div>
        ))}
